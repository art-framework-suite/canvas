...
namespace {
  ...
  struct HoursPerWorker {
    ...
    void aggregate(HoursPerWorker const& b)
    {
      if ( name_ != b.name_ ) return;
      hours_ += b.hours_;
    }
  };
...
namespace std {
  ...
  ostream& operator<<(ostream& os, pair<string, unsigned> const& p)
  {
    os << '[' << p.first << ',' << p.second << ']';
    return os;
  }

  ostream& operator<<(ostream& os, pair<cet::map_vector_key,string> const& entry)
  {
...
BOOST_AUTO_TEST_CASE (string)
{
  ...
  BOOST_REQUIRE_EXCEPTION(r.get<std::string>(),
                          art::Exception,
