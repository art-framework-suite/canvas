...
namespace art {
  ...
  namespace detail {
    ...
    struct are_cv_compatible {
      ...
      static constexpr bool value {std::is_base_of<T1P, T2P>::value || std::is_same<T1P, T2P>::value};
    };

    template <typename TO, typename FROM>
    constexpr
    std::enable_if_t< are_cv_compatible<TO, FROM>::value, std::add_pointer_t<std::remove_pointer_t<TO>> >
    addr(FROM& from) { return &from; }

    template <typename TO, typename FROM>
    constexpr
    std::enable_if_t<!are_cv_compatible<TO, FROM>::value && are_cv_compatible<FROM, TO>::value, std::add_pointer_t<std::remove_pointer_t<TO>> >
    addr(FROM& from) { return &dynamic_cast<std::add_lvalue_reference_t<std::remove_pointer_t<TO>>>(from); }

    template <typename TO, typename FROM>
    struct EnsurePointer {
      constexpr TO operator()(FROM& from) const { return detail::addr<TO>(from); }
      constexpr TO operator()(FROM const& from) const { return detail::addr<TO>(from); }
    };

    template <typename TO, typename PFROM>
    struct EnsurePointer<TO, PFROM*> {
      constexpr TO operator()(PFROM* from) const { return detail::addr<TO>(*from); }
    };
  }
}
...
art::ensurePointer(InputIterator it)
  try
    {
      static_assert(std::is_pointer<WANTED_POINTER>::value, "Supplied template argument is not a pointer.");
