...
namespace {
  struct Dummy {
    ...
    virtual ~Dummy() {}
    bool
    operator==(Dummy const& iRHS) const
    {
      return this == &iRHS;
    }
    bool
    operator<(Dummy const& iRHS) const
    {
      return this->address() < iRHS.address();
    }
    void const*
    address() const
    {
...
testPtr::constructTest()
{
...

  {
    ...
    CPPUNIT_ASSERT(dummy2Ptr.get() ==
                   static_cast<const Dummy*>(dummy2Ptr.get()));
  }
}

void
testPtr::comparisonTest()
{
...
testPtr::getTest()
{
  ...
  IntCollection const* wptr =
    dynamic_cast<IntCollection const*>(wrapper.product());
...

  {
    ...
    SDCollection const* wptr =
      dynamic_cast<SDCollection const*>(wrapper.product());
