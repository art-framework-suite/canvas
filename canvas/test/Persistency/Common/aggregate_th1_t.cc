...
namespace {
  ...
  struct RandomNumberHandler {
    ...
    float operator()() { return numDist_(engine_); }
  };

  struct Error {
    auto operator()(float const val)
    {
      return std::sqrt(val);
    }
  };
  ...
  struct LinearAdd {
    auto operator()(float const val1, float const val2)
    {
      return val1+val2;
    }
  };
...
BOOST_AUTO_TEST_CASE (th1f)
{
  ...
  TH1F h1 {"title1", "name1", nBins, 0, 10.}; h1.Sumw2();
  TH1F h2 {"title2", "name2", nBins, 0, 10.}; h2.Sumw2();
...
BOOST_AUTO_TEST_CASE (th2f)
{
  ...
  TH2F h1 {"title1", "name1", nBinsX, 0, 10., nBinsY, -5, 15}; h1.Sumw2();
  TH2F h2 {"title2", "name2", nBinsX, 0, 10., nBinsY, -5, 15}; h2.Sumw2();
  ...
  for (int i{0} ; i != nBinsX ; ++i) {
    ...
    cet::transform_all(tmp1err, tmp2err, std::back_inserter(tmp3err), QuadraticAdd{});

    h1d.push_back(std::move(tmp1d));  h1err.push_back(std::move(tmp1err));
    h2d.push_back(std::move(tmp2d));  h2err.push_back(std::move(tmp2err));
    h3d.push_back(std::move(tmp3d));  h3err.push_back(std::move(tmp3err));
  }
