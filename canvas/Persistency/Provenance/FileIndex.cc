...
constexpr
FileIndex::Element::invalidEntry;
...
void
FileIndex::
addEntry(EventID const& eID, EntryNumber_t const entry)
{
...
void
FileIndex::
addEntryOnLoad(EventID const& eID, EntryNumber_t const entry)
{
  entries_.emplace_back(eID, entry);
  resultCached() = false;
}
...
void
FileIndex::
print_event_list(ostream& os) const
{
