...
ProcessHistory::
ProcessHistory(ProcessHistory const& rhs)
  : data_(rhs.data_)
...
// Note: We do give the basic exception safety guarantee.
ProcessHistory::
ProcessHistory(ProcessHistory&& rhs)
...
ProcessHistory&
ProcessHistory::
operator=(ProcessHistory const& rhs)
{
  if (this != &rhs) {
    data_ = rhs.data_;
    transients_ = rhs.transients_;
...
id() const
{
  ...
  // Note: threading: with the mutex already locked, so we use recursive_mutex.
...
bool
ProcessHistory::
getConfigurationForProcess(string const& name, ProcessConfiguration& config) const
{
  lock_guard<recursive_mutex> sentry(mutex_);
