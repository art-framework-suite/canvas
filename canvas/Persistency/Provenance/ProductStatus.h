...
namespace art {
  ...
  namespace productstatus {
    ...
    inline ProductStatus dummyToPreventDoubleCount() {return 0x3;} // Product is a dummy placeholder object with an invalid range set to prevent double-counting when combining run/subrun products
    ...
    inline bool dummyToPreventDoubleCount(ProductStatus status) {return status == dummyToPreventDoubleCount();}
