...
namespace {
  ...
  auto form_translation_map(cet::exempt_ptr<art::BranchIDLists const> branchIDLists)
  {
...
namespace art {
  ...
  ProductIDStreamer::ProductIDStreamer(cet::exempt_ptr<BranchIDLists const> branchIDLists) :
    branchIDLists_{branchIDLists},
    branchIDListsIndices_{form_translation_map(branchIDLists_)}
  {}

  void
  ProductIDStreamer::setBranchIDLists(cet::exempt_ptr<BranchIDLists const> bidLists)
  {
  ...
  ProductIDStreamer::operator()(TBuffer& R_b, void* objp)
  {
	...
	auto const oldBIDListsIndex = branchIDListsIndices_[oldProcessIndex-1];
