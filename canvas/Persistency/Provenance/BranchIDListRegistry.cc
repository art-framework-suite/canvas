...
void
art::BranchIDListRegistry::mergeFromFile(BranchIDLists const& file_bidlists,
                                         std::string const& fileName)
{
...
//
//   Push back an entry into the BranchIDListRegistry (which is either
//   empty or read from the first input file) for the products
//   produced by the current process.
//
void
art::BranchIDListRegistry::expand(ProductList const& products)
{
  CET_ASSERT_ONLY_ONE_THREAD();
