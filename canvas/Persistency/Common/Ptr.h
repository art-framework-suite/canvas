...
public:
  ...
  Ptr(H const& handle, typename Ptr<T>::key_type key)
    : core_(handle.id(),
            detail::ItemGetter<T,
                               std::remove_const_t<std::remove_pointer_t<decltype(handle.product())>>
                              >()(handle.product(), key),
            nullptr)
    , key_(key)
  {
  ...
  // 4.
  template<typename U>
  Ptr(Ptr<U> const& pu, std::enable_if_t<std::is_base_of<T, U>::value>* dummy = nullptr)
    : core_(
          pu.id()
  ...
  template<typename U>
  Ptr(Ptr<U> const& pu, std::enable_if_t<std::is_base_of<U, T>::value>* dummy = nullptr)
  ...
  operator->() const
  {
    if (core_.productPtr() == nullptr) {
      ...
      if (prod == nullptr) {
        ...
        e << "A request to resolve an Ptr to a product containing items of type: "
...
public:
  ...
  T const*
  operator()(cet::map_vector<T> const* product, typename Ptr<T>::key_type iKey) const
  {
    assert(product != nullptr);
